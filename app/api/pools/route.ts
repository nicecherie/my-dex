...
export async function POST(request: Request) {
  try {
    ...
    // 计算价格比例
    const token0BalanceFormatted = parseFloat(
      formatUnits(token0Balance, token0Info.decimals)
    )
    const token1BalanceFormatted = parseFloat(
      formatUnits(token1Balance, token1Info.decimals)
    )
