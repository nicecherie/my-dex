...
import { useMultiplePoolsData } from './usePoolsData'
...
export const usePools = () => {
  ...
  // 准备池子地址列表用于批量获取数据
  const poolAddresses =
    poolsData && Array.isArray(poolsData)
  ...
  // 获取所有池子的真实链上数据
  const {
    poolsData: realPoolsData,
    loading: poolDataLoading,
    error: poolDataError
  } = useMultiplePoolsData(poolAddresses)
