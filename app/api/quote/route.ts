...
async function findPoolsForPair(tokenIn: string, tokenOut: string) {
  ...
  const { data: pools, error } = await supabase
    .from('pools')
    .select(
      'address, pool_index, fee, liquidity, token0, token1, sqrt_price_x96'
    )
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            '缺少 WETH 地址配置（NEXT_PUBLIC_WETH9_ADDRESS / WETH9_ADDRESS）'
        },
        { status: 500 }
      )
    }
    ...
    if (!pools || pools.length === 0) {
      return NextResponse.json(
        {
          error: `No pool found for pair ${tokenIn}-${tokenOut}`,
          msg: '未找到交易对'
        },
        { status: 404 }
      )
