...
export function useSwap() {
  ...
  const getDefaultSqrtPriceLimit = useCallback(
    (tokenIn: string, tokenOut: string): bigint => {
  ...
  const resolveAvailableRoute = useCallback(
    async (tokenA: string, tokenB: string) => {
      ...
      const matchedPools = Array.from(pools).filter((pool) => {
        ...
        return (
          (aAliases.has(p0) && bAliases.has(p1)) ||
          (aAliases.has(p1) && bAliases.has(p0))
        )
      })
    ...
    async (params: SwapParams): Promise<QuoteResult | null> => {
      ...
      try {
        ...
        const tokenInDecimals =
          params.tokenInDecimals ?? tokenInObj?.decimals ?? 18
  ...
  // 授权代币 配合 useTokenAllowance 使用，如果授权值 < 输入数量，则需要先授权。授权后才能执行交换。
  ...
  const executeSwap = useCallback(
    async (params: SwapParams) => {
      ...
      const tokenOutDecimals =
        params.tokenOutDecimals ?? tokenOut?.decimals ?? 18
      ...
      const resolvedRoute = await resolveAvailableRoute(
        actualTokenIn,
        actualTokenOut
      )
      ...
      if (!isNativeTokenIn && !shouldUsePermit) {
        ...
        if (BigInt(allowance) < amountInWei) {
          throw new Error(
            '授权不足，请先点击 Approve（会导致 transferFrom 失败）'
          )
        }
      }
